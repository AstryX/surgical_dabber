...
pred_labels, final_contours, loaded_image = predictImageLabels(params_path, predict_num)
before_top_pool_id, top_pool_deepest_point_id, top_pool_volume = findOptimalDestination(before_z, after_z, 
...
step_x = (max_range_x * 2) / image_size_cols
step_y = (max_range_y * 2) / image_size_rows

center_x = step_x * (image_size_cols/2)
center_y = step_y * (image_size_rows/2)

#TODO Add extra to base z to account for depth
goal_x_idx = int(math.floor(top_pool_deepest_point_id / image_size_cols))
goal_x = step_x * goal_x_idx - center_x 

goal_y_idx = int(top_pool_deepest_point_id % image_size_cols)
goal_y = step_y * goal_y_idx - center_y
